...
def load_data(opt):
    ...
    with open(split_file) as csvfile:
        ...
        for row in csvReader:
            ...
            if row[2] == "training":
                ...
                val_files[domain].append({"image": image_name, "label": label_name})
            elif row[2] == "test":
                test_files.append({"image": image_name, "label": label_name})

    # check if all files exist
    for file_dict in train_files[0] + train_files[1] + val_files[0] + val_files[1] + test_files:
    ...
    logger.info("Number of images in validation (source) = {}".format(len(val_files[0])))
    logger.info("Number of images in validation (target) = {}".format(len(val_files[1])))
    ...
    if opt.debug:
        ...
        logger.info("validation set (source) = {}".format(val_files[0]))
        logger.info("validation set (target) = {}".format(val_files[1]))
        logger.info("test set       (target) = {}".format(test_files))
